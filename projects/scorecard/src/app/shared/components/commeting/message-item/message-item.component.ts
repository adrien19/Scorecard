...
@Component({
  ...
  template:`
    ...
    <span *ngFor="let word of messageWords" style="white-space:pre">
      <a *ngIf="word.link" [routerLink]="word.link">{{ word.text }}</a>
      <span *ngIf="!word.link">{{ word.text }}</span>
    </span>
...
export class MessageItemComponent implements OnInit {
  ...
  @Input('message') messageString: { message: string, mentions: IUserHolder[] };
  ...
  parse() {
    ...
    let words: string[] = this.messageString.message.split(' ');
  ...
  generateMentionLink(word: string): string {
    ...
    const userId = this.messageString.mentions? this.messageString.mentions[0].userId : null;
    if (userId) {
      console.log("mention exist", this.messageString.mentions);
      return `users/${userId}`;
    }
    console.log("mention doesn't exist");
